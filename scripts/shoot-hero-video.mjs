// Hero-video landing screenshots. Uses the CRR-cached Playwright + chromium-1223.
// Usage: node scripts/shoot-hero-video.mjs <baseUrl> <outDir>
...
const OUT = process.argv[3] || "/home/codex/Projects/nucleus/screenshots/landing";
const EXE = "/home/codex/.cache/ms-playwright/chromium-1223/chrome-linux64/chrome";

mkdirSync(OUT, { recursive: true });

async function shoot(label, width) {
  ...
  // let the AskDemo typing animation reach the answer + scroll reveals settle
  ...
  await page.screenshot({ path: `${OUT}/hero-video-${label}.png`, fullPage: true });

  await browser.close();
  return { status: resp ? resp.status() : "none", errors };
}

const d = await shoot("desktop", 1280);
const m = await shoot("mobile", 390);
