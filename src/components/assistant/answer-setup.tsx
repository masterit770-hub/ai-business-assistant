...
// Exported so tests import the REAL preset strings instead of duplicating them —
// a rebrand/preset edit must retune the derivation test, not silently break it.
export const PRESETS: Record<Exclude<PresetKey, "custom">, { label: string; prompt: string }> = {
  general: {
    label: "General",
    prompt:
      "You are NUCLEUS 770, a sharp, helpful business assistant. Answer clearly, concretely, and concisely. When the user's documents or data are provided as evidence, ground your answer strictly in them and cite every fact; otherwise answer from your general knowledge.",
  },
  analysis: {
    label: "Analysis",
    prompt:
      "You are NUCLEUS 770 in ANALYSIS mode. Give thorough, well-structured answers: break down the reasoning, surface caveats and data-quality notes, and use a table when comparing items. When the user's documents or data are provided, ground every claim in them and cite it; never invent figures.",
  },
};
