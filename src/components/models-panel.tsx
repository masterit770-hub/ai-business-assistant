...
export function ModelsPanel() {
  ...
  // Cloud (Claude / Anthropic) key + optional model override (the paste-a-key path).
  ...
  async function save(): Promise<boolean> {
    ...
    try {
      ...
      const body: Record<string, string> = {
        ...
        // The Cloud lane is Claude-only now; clear any stored provider override so the
        // engine falls back to its configured Claude/Anthropic key. (UI-only change —
        // the live answer path doesn't read this; see the panel copy.)
        cloud_provider: "",
  ...
  return (
    <div className="space-y-5">
      ...
      {/* data-hydrated: flips to "1" once the async settings GET has landed (applyServer set
          the `loaded` baseline). Interactions BEFORE that point get clobbered by the load —
          a journey/user filling fields pre-hydration loses them and Save stays disabled.
          Automation must wait for [data-hydrated="1"] before touching this panel. */}
      <div className="rounded-2xl border border-line bg-surface shadow-soft" data-testid="model-section" data-hydrated={loaded ? "1" : "0"}>
        ...
        <div className="space-y-5 p-6">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-ink">Cloud model — Claude (Anthropic)</h3>
            </div>
            <p className="text-sm text-faint">
              Nucleus runs on Claude (Anthropic). Paste your Anthropic API key to store your own.
              Note: in the hosted deployment the live answer key is configured on the server
              (<code className="rounded bg-canvas px-1 py-0.5">ANTHROPIC_API_KEY</code>) — this field
              stores an override used by the in-process engine.
            </p>

            {/* Claude API key (write-only) — always visible */}
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder={cloudKeySet ? "Saved — type to replace" : "Paste your Anthropic API key (sk-ant-…)"}
                spellCheck={false}
                autoComplete="off"
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-ink" htmlFor="cloud-model">
                Model <span className="font-normal text-faint">(optional)</span>
              </label>
              <input
                id="cloud-model"
                value={cloudModel}
