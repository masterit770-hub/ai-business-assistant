...
export type AnswerResult = {
  ...
  // The model that ACTUALLY answered this turn (the resolved model string sent to the
  // provider). OPTIONAL — the Messages-API engine sets it so a test/journey can assert
  // which model ran (guarding the "prod silently ran Haiku" false-green); other engines
  // may leave it undefined. Never a promise/config value — it reflects the real call.
  model?: string;
...
export async function answerQuestion(
  ...
  ctx: { ownerId?: string; role?: string; isDemo?: boolean; history?: Turn[]; chatId?: string | null } = {}
...
async function runAnswerPipeline(
  ...
  const route = await routeQuestion(question, {
    ...
    isAdmin,
    // Per-chat scoping (migration 014): scope the routing doc/table catalog to this chat.
    chatId: ctx.chatId,
  });
  ...
  if (route.sources.includes("structured")) {
    try {
      const structured = await answerStructured(question, includeBundled, structuredScope, ctx.chatId);
    ...
    if (supabaseEnabled()) {
      const uploadedLists = await Promise.all(
        embeddings.map((emb, i) =>
          // Per-chat scoping (migration 014): when chatId is present, hybridSearch
          // scopes to (owner + chat) OR legacy (session_id IS NULL). Null = all.
          hybridSearch(scopeOwner, ctx.role === "admin", emb, retrievalQueries[i], 12, ctx.chatId)
        )
      );
