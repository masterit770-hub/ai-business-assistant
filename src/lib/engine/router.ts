...
export async function routeQuestion(
  ...
  // caller's own tables.
  ctx: { ownerId?: string; history?: Turn[]; isDemo?: boolean; isAdmin?: boolean; chatId?: string | null } = {}
): Promise<RoutePlan> {
  ...
  const [mem, durable] = await Promise.all([
    ...
    // Per-chat scoping (migration 014): when chatId is set, the durable doc catalog
    // only lists docs for this chat + legacy (session_id IS NULL), so the router routes
    // based only on the docs this chat can actually see.
    listUploadedDocsDurable(ctx.ownerId, ctx.chatId).catch(() => []),
  ]);
  ...
  // Per-chat scoping (migration 014): when chatId is present, hydrate only the
  // active chat's rows + legacy into the structured catalog.
  if (scope) await hydrateUploadedTables(scope.ownerId, !!scope.isAdmin, ctx.chatId);
