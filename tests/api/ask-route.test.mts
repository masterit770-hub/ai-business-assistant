...
before(() => {
  ...
  mockExports("@/lib/engine/request-context", {
    runWithOwner: async (id: string, fn: () => unknown) => { lastAnswerOpts = { ownerCtx: id }; return fn(); },
    // currentOwner is read by settings.ts (pulled in transitively via the answer engines);
    // the mock must expose it or the route's static import graph fails to instantiate.
  ...
  // The route imports the agentic + messages engines at the top level; stub them so the
  // heavy real modules (and their transitive deps) don't load for this default-lane suite.
  mockExports("@/lib/engine/answer-agentic", { answerAgentic: async (q: string) => ({ answer: `A:${q}` }) });
  mockExports("@/lib/engine/answer-messages", { answerMessages: async (q: string) => ({ answer: `A:${q}` }) });
  mockExports("@/lib/engine/answer-local", { answerLocal: async (q: string) => ({ answer: `A:${q}` }) });
  // model_mode drives lane selection now; this default suite exercises the cloud path.
  mockExports("@/lib/engine/settings", { getModelMode: async () => "cloud" });
  // Knowledge Space bookkeeping seam the route now calls when a space_id is present.
