...
describe("ModelsPanel", () => {
  ...
  it("the cloud Model input is disabled until a provider is chosen", async () => {
    mockFetch({ "/api/settings": { json: model({ cloud_provider: "" }) } });
    render(<ModelsPanel />);
    await waitFor(() => expect(screen.getByTestId("cloud-model-input")).toBeInTheDocument());
    expect(screen.getByTestId("cloud-model-input")).toBeDisabled();

    await userEvent.selectOptions(screen.getByTestId("cloud-provider-select"), "openai");
    expect(screen.getByTestId("cloud-model-input")).toBeEnabled();
    // The api-key + base-url fields appear once a provider is selected.
    expect(screen.getByTestId("cloud-api-key-input")).toBeInTheDocument();
    expect(screen.getByTestId("cloud-base-url-input")).toBeInTheDocument();
  });
