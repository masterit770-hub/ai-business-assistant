...
test("friendlyAskError(rate limit) → the usage-limit line (a 429 is NOT a key problem)", () => {
  const line = friendlyAskError(new Error("deepseek (deepseek-chat) 429: rate limit"));
  // It's the clear "Claude key has reached its usage limit" line — NOT the
  // "fix your key in Settings → Models" guidance (a 429 is a quota issue, not a bad key).
  assert.match(line, /usage limit/i);
